...
# Detectar si un Pokémon tiene pocos movimientos
def tiene_poca_info(moves):
    return not isinstance(moves, str) or len(moves.split()) < 10

pokemon["poca_info"] = pokemon["moves"].apply(tiene_poca_info)
...
x = vec.fit_transform(pokemon["moves"])

print("\nEstos son los datos en forma de matriz:")
print(x.toarray())

print("\nTotal de palabras del vocabulario:")
...
# Reasignar a cluster especial a Pokémon con poca información
pokemon.loc[pokemon["poca_info"], "Grupo"] = 99
...
resultado_final["Grupo"] = pokemon["Grupo"]  # Agrega la versión actualizada del grupo

resultado_final.to_csv("pokemon_clusters_p1_TfIDF.csv", index=False)
pokemon[["Pokemon", "Tipo", "Grupo"]].to_csv("pokemon_clusters_p1_TfIDF_Filtered.csv", index=False)

print("\nArchivo 'pokemon_clusters_p1_TfIDF.csv' guardado exitosamente!")
print("\nArchivo 'pokemon_clusters_p1_TfIDF_Filtered.csv' guardado exitosamente!")

# Mostrar información de los clusters
print("\nInformación de los clusters (incluye Grupo 99):")
for grupo_id in sorted(pokemon["Grupo"].unique()):
    cluster_df = pokemon[pokemon["Grupo"] == grupo_id]
    print(f"Cluster {grupo_id}: {len(cluster_df)} Pokémon")
    if len(cluster_df) > 0:
        print(f"  Ejemplos: {list(cluster_df['Pokemon'].head(3))}")


print("\nAnálisis de clusters con tipo dominante:")
for i in sorted(pokemon["Grupo"].unique()):
    cluster_df = pokemon[pokemon["Grupo"] == i]
    tipos = []
